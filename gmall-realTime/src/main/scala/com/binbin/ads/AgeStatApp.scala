...
object AgeStatApp {
  def main(args: Array[String]): Unit = {
    ...
    val topicName = "DWD_ORDER_WIDE_AGE"
    val groupId = "AgeStatApp"
    ...
    val ageOrderWideDS: DStream[(String, Double)] = orderWideDS.map {
      orderWide =>
        (orderWide.user_age_group, orderWide.final_total_amount)
    }

    val ageIdReduceDS: DStream[(String, Double)] =
      ageOrderWideDS.reduceByKey(_ + _)

    //  执行写入
    ageIdReduceDS.foreachRDD { rdd =>
      ...
      if (amountArray.nonEmpty) {
        ...
        DB.localTx(implicit session => {
          // 保存age信息
          amountArray.foreach {
            case (age, finalTotalAmount) =>
              SQL("insert into age_amount_stat values (?,?,?)")
                .bind(MyDateUtils.getNowYMDHMS(), age, finalTotalAmount)
                .update()
                .apply()
